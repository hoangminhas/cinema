...
<body>
    ...
    <div class="Registro">
        <form method="post" action="{{ route('register') }}">
            @csrf
            <span class="fontawesome-user"></span><input type="text" value="{{ old('name') }}" name="name"
                placeholder="name" autocomplete="off">
            {{-- <p style="color: red">{{($errors->has('name') ? $errors->first('name') : "")}}</p> --}}
            <span class="fontawesome-envelope-alt"></span><input type="email" value="{{ old('email') }}" id="email"
                name="email" placeholder="Email" autocomplete="off">
            {{-- <p style="color: red">{{$errors->has('email') ? $errors->first('mail') : ""}}</p> --}}
            <span class="fontawesome-lock"></span><input type="password" name="password" id="password"
                placeholder="Password" autocomplete="off">
            {{-- <p style="color: red">{{$errors->has('password') ? $errors->first('password') : ""}}</p> --}}
            <span class="fontawesome-lock"></span><input type="password" name="confirmPassword"
